...
function ValidatorUpdateIsValid() { var i; for (i = 0; i < Page_Validators.length; i++) { if (!Page_Validators[i].isvalid) { Page_IsValid = false; Page_BlockSubmit = true; return; }
}
Page_IsValid = true; }
...
function Page_ClientValidate() { var i,ctrl; for (i = 0; i < Page_Validators.length; i++) { ValidatorValidate(Page_Validators[i]); }
...
function ValidatorOnChange(controlID) { var cont = dom_getElementByID(controlID); var vals = cont.Validators; var i; for (i = 0; i < vals.length; i++) { ValidatorValidate(vals[i]); }
